...
| `bun src/index.js --check app.md` | Check heading and text-control IDs for collisions, print line-by-line details, and exit. |
...
Heading IDs share the same selector namespace as explicitly named controls, so
avoid name collisions between them. For example, `## Write Status` generates
`#write-status` and must not be used together with a control such as
`text#write-status`. Otherwise, `$('#write-status')` may select the
heading instead of the text field and updates can appear to do nothing. Rename
either the heading or the control so every selectable ID is unique.
