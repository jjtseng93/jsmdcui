...
test("fenced text controls retain quoted inline keyup code after the identity", () => {
  const markdown = '```text#myid.field @keyup="first(); second(event); show(\\"done\\")"\nvalue\n```\n';
  ...
  expect(declarations[0].events.get("keyup")).toEqual({
...
test("prevent modifier prepends preventDefault to inline event code", () => {
  const markdown = '```text#myid @keyup.prevent="submit(event)"\nvalue\n```\n';
  const handler = fenceEventMap(markdown).get("myid").events.get("keyup");
...
test("WUI writes keyboard code as native inline handlers with a mobile beforeinput fallback", () => {
  const markdown = '```text#myid.field @keydown.prevent="guard(event)" @keyup.prevent="update(); show(\\"done\\", event)"\nvalue\n```\n';
  ...
  expect(html).toContain('if(event.key!==&quot;Unidentified&quot;){event.preventDefault();guard(event)}"');
  ...
  expect(html).toContain('onkeyup="event.preventDefault();update(); show(&quot;done&quot;, event)"');
...
test("TUI runs inline keyup code after editing the target text block", async () => {
  const dir = await mkdtemp(join(tmpdir(), "jsmdcui-keyup-"));
  const markdownPath = join(dir, "app.md");
  const markdown = [
    '```text#field @keyup="record(event)"',
    "a",
    "```",
    "",
    "```text#status",
    ...
    "  $('#status').val(`${event.key}:${event.target.value}`);",
    "}",
    "```",
    "",
  ].join("\n");
  ...
  try {
    ...
    terminal.write("x");
    await waitFor(() => Bun.stripANSI(output).includes("x:ax"));
    terminal.write("\x11");
    await Promise.race([proc.exited, Bun.sleep(2000)]);
    expect(Bun.stripANSI(output)).toContain("x:ax");
  } finally {
